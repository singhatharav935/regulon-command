...
// ── Sandbox.co.in: fetch GST filing history (requires SANDBOX_API_KEY) ────────
async function fetchGSTFilingHistory(gstin: string): Promise<Record<string, unknown>[] | null> {
  ...
  try {
    // Sandbox GST compliance — returns 24-month filing table
    const res = await fetch(
      `https://api.sandbox.co.in/gst/compliance/taxpayer/details/${gstin}`,
      {
        headers: {
          "x-api-key": apiKey,
          "x-api-version": "1.0",
    ...
    if (res.ok) {
      const d = await res.json();
      // filing_table is an array of {return_type, tax_period, date_of_filing, status}
      return d?.filing_table || null;
    }
  } catch { /* fall through */ }
  return null;
}

// ── MCA: fetch company master data ────────────────────────────────────────────
async function fetchMCAData(cin: string): Promise<Record<string, unknown> | null> {
  ...
  try {
    const res = await fetch(
      `https://api.sandbox.co.in/mca/company/searchcompany?company_id=${cin}`,
      {
        headers: { "x-api-key": apiKey, "x-api-version": "1.0", Accept: "application/json" },
      }
    );
    if (res.ok) {
      const d = await res.json();
