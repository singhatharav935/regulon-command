...
const path = '/Users/atharavsingh/Desktop/SANNIDH_MASTER/frontend/src/pages/ExternalCADashboardReal.tsx';
let content = fs.readFileSync(path, 'utf8');

// The replacement HTML template. It removes CADashboardSubNav and activeZone tabs.
const newLayoutString = `          {/* ─── NEW MINIMAL HUB & SPOKE COMMAND CENTER ─── */}
          <div className="flex flex-col xl:flex-row gap-6 items-start w-full">
            ...
            <div className="flex-1 min-w-0 space-y-8 w-full">
              ...
              <motion.div className="space-y-4">
                ...
                {/* Dashboard Static Metrics Skeleton / Real Connection Array */}
              ...
              <motion.div className="space-y-4 pt-6 mt-6 border-t border-border/30">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <Card className="hover:border-indigo-500/50 hover:bg-indigo-500/5 transition-all text-left">
            ...
            <div className="w-full xl:w-[400px] flex-shrink-0 space-y-6">
               ...
               <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                 <RegulatoryNewsRuleImpact
                   ...
                   apiEndpoint={\`\${CA_API}/api/v1/regulatory/news\`}
                   aiEnabled={true}
                   caId={caId}
                 />
               </motion.div>
...
// Extract everything from <div className="flex gap-6 items-start"> to its closing </div>
const startIndex = content.indexOf('<div className="flex gap-6 items-start">');
// We need to carefully isolate the activeZone conditions for Clients and Operations so they become Modals overlying the command center.
// Since the layout was nested inside that flex container, we'll replace the main Command Center flex block.
// Let's actually just replace from line 964 to 1400.

const lines = content.split('\n');
// We want to slice out the `CADashboardSubNav` and all `activeZone` conditional rendering blocks.
// I will build a completely new string.

fs.writeFileSync('/Users/atharavsingh/Desktop/SANNIDH_MASTER/frontend/scripts/build-layout.js', `
const fs = require('fs');
let code = fs.readFileSync('${path}', 'utf8');

// Safely remove CADashboardSubNav import if wanted (not strictly necessary but clean).
code = code.replace("import CADashboardSubNav", "// import CADashboardSubNav");

// Remove the import of ArrowRight if not present, and import it from lucide-react.
