...
class EnhancedAuthService {
  ...
  private getOrCreateDeviceId(): string {
    let deviceId = localStorage.getItem('regulon_device_id');
    if (!deviceId) {
      deviceId = generateSecureToken(16);
      localStorage.setItem('regulon_device_id', deviceId);
    }
    return deviceId;
  }
  ...
  private clearStoredAuth(): void {
    ...
    localStorage.removeItem('regulon_refresh_token');
  ...
  private trackDeviceFingerprint(): void {
    ...
    localStorage.setItem('regulon_device_fingerprint', JSON.stringify(fingerprint));
  }
  ...
  private detectSuspiciousActivity(): boolean {
    const storedFingerprint = localStorage.getItem('regulon_device_fingerprint');
  ...
  private storeAuthData(response: AuthResponse, remember: boolean): void {
    ...
    storage.setItem('regulon_auth_token', response.token);
    storage.setItem('regulon_refresh_token', response.refresh_token);
    localStorage.setItem('regulon_user', JSON.stringify(response.user));
    localStorage.setItem('regulon_session', JSON.stringify(response.session));
  ...
  async refreshToken(): Promise<void> {
    const refreshToken = localStorage.getItem('regulon_refresh_token') || 
                        sessionStorage.getItem('regulon_refresh_token');
    ...
    try {
      ...
      const storage = localStorage.getItem('regulon_auth_token') ? localStorage : sessionStorage;
      storage.setItem('regulon_auth_token', response.token);
      storage.setItem('regulon_refresh_token', response.refresh_token);
  ...
  isAuthenticated(): boolean {
    const token = localStorage.getItem('regulon_auth_token') || 
                  sessionStorage.getItem('regulon_auth_token');
