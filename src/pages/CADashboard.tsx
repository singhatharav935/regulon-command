...
const LiveAIDraftingEngine = () => {
  ...
  const processUploadedDocument = async () => {
    ...
    try {
      ...
      addAgentLog(`📝 Reading text from uploaded notice: ${uploadedFile.name}...`);
      
      await new Promise(resolve => setTimeout(resolve, 1000));
      addAgentLog(`👁️ SANNIDH Vision OCR Engine initialized successfully.`);
      addAgentLog(`✅ OCR extracted 3,412 characters from scanned document.`);
      ...
      const newDemoTask = {
        ...
        progress: 100,
        createdAt: new Date().toISOString(),
        description: `Drafted GSTR-2B rebuttal for uploaded file: ${uploadedFile.name}. Ready for CA Review.`,
        result: {
          documentType: "GST Notice Rebuttal (SCN-829412)",
          pages: 4,
          sections: ["1. Preliminary Objections", "2. Ledger Reconciliation Matrix", "3. Section 16(4) Defense", "4. Verification Request"],
          generatedAt: new Date().toISOString()
        }
      };
  ...
  const handleSendForClientSignature = async () => {
    ...
    try {
      ...
      // Update Task list in demo dashboard
      setAiTasks(prev => prev.map(t => 
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="drafting" className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Notice Input & Digital Portal Inbox */}
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <h3 className="text-lg font-semibold text-cyan-300">AI-Generated Notice Response</h3>
                    {generatedDraft && (
                      <div className="flex rounded-lg bg-card/60 p-0.5 border border-border/50">
                        <Button
                          size="sm"
                          variant={draftViewMode === 'text' ? 'default' : 'ghost'}
                          className={`h-7 px-3 text-xs ${draftViewMode === 'text' ? 'bg-cyan-600 text-white hover:bg-cyan-500' : 'text-muted-foreground'}`}
                          onClick={() => setDraftViewMode('text')}
                        >
                          📝 Raw Text
                        </Button>
                        <Button
                          size="sm"
                          variant={draftViewMode === 'pdf' ? 'default' : 'ghost'}
                          className={`h-7 px-3 text-xs gap-1 ${draftViewMode === 'pdf' ? 'bg-cyan-600 text-white hover:bg-cyan-500' : 'text-muted-foreground'}`}
                          onClick={() => setDraftViewMode('pdf')}
                        >
                          📄 Visual PDF Preview
                          {draftApprovalStatus !== 'pending' && <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />}
                        </Button>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Drafting views */}
                  {isProcessing ? (
                    <div className="bg-card/30 border border-cyan-500/20 rounded-lg h-[400px] flex flex-col items-center justify-center p-6 text-center space-y-4">
                      <Loader className="w-10 h-10 animate-spin text-cyan-500" />
                      <div className="space-y-1">
                        <p className="text-sm font-semibold text-cyan-300">Agent Swarm Active</p>
                        <p className="text-xs text-muted-foreground">Running statutory cross-reference audits and generating compliance draft reply...</p>
                      </div>
                    </div>
                  ) : !generatedDraft ? (
                    <div className="bg-card/30 border border-cyan-500/20 rounded-lg h-[400px] flex flex-col items-center justify-center p-6 text-center space-y-2 text-muted-foreground">
                      <Bot className="w-12 h-12 opacity-30 text-cyan-400" />
                      <p className="text-sm font-medium">No notice response generated yet</p>
                      <p className="text-xs max-w-[300px]">Select a digital portal notice on the left or upload a custom notice file to trigger the autonomous AI drafting swarm.</p>
                    </div>
                  ) : draftViewMode === 'text' ? (
                    <Textarea
                      placeholder="AI will generate legal draft here based on notice..."
                      value={generatedDraft}
                      onChange={(e) => setGeneratedDraft(e.target.value)}
                      className="bg-card/50 border-cyan-500/20 h-[400px] font-mono text-xs leading-relaxed"
                    />
                  ) : (
                    /* High-Fidelity Paper Document Letterhead Preview */
                    <div className="bg-slate-50 text-slate-800 rounded-xl p-8 h-[400px] overflow-y-auto font-serif shadow-2xl relative border border-slate-300 flex flex-col space-y-4 select-text">
                      {/* Letterhead Header */}
                      <div className="border-b-2 border-slate-900 pb-4 text-center md:text-left md:flex md:justify-between md:items-start space-y-2 md:space-y-0">
                        <div className="text-left">
                          <h1 className="text-lg font-bold text-slate-900 tracking-wider">SANNIDH & CO.</h1>
                          <p className="text-[10px] text-slate-600 font-sans tracking-wide uppercase">Chartered Accountants & Legal Advisors</p>
                          <p className="text-[8px] text-slate-500 font-sans leading-tight mt-1">
                            ICAI Firm Reg. No: 1029481W | ISO 27001 Certified Practice<br />
                            Office: 402, Connaught Place, New Delhi - 110001
                          </p>
                        </div>
                        <div className="text-center md:text-right font-sans">
                          <span className="border border-red-500 text-red-500 font-bold text-[8px] px-2 py-0.5 rounded block mb-1">
                      ...
                      {/* Letter content */}
                      <div className="text-[11px] leading-relaxed space-y-4 pr-2 font-serif text-slate-800 whitespace-pre-wrap text-left">
                        {generatedDraft}
                      </div>

                      {/* Signatures and Seals */}
                      <div className="border-t border-slate-300 pt-6 mt-8 space-y-6 font-sans">
                        <div className="flex justify-between items-end flex-wrap gap-4">
                          <div className="text-left">
                            <p className="text-[10px] font-bold text-slate-900">For and on behalf of SANNIDH & CO.</p>
                            <p className="text-[9px] text-slate-600">Chartered Accountants</p>
                            
                            {/* CA Signature Seal */}
                            {draftApprovalStatus !== 'pending' ? (
                              <div className="my-2 p-1.5 rounded bg-green-500/10 border border-green-500/30 text-green-700 max-w-[200px]">
                                <p className="font-serif italic text-xs font-bold text-green-800">Rajesh Kumar</p>
                                <p className="text-[7px] font-sans text-green-600">FCA Member No: 509284<br />Signed via Digital Signature Certificate (DSC)</p>
                              </div>
                            ) : (
                          ...
                          <div className="text-right flex flex-col items-end">
                            ...
                            <p className="text-[9px] text-slate-500">Second Signatory</p>
                          </div>
                        </div>

                        {/* WORM Security Blockchain Stamp */}
                        {draftApprovalStatus !== 'pending' && (
                          <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg flex items-start gap-2.5 text-left text-emerald-850 mt-4 shadow-sm">
                            <Shield className="w-5 h-5 text-emerald-600 flex-shrink-0 mt-0.5" />
                            <div className="space-y-0.5">
                              <p className="text-[9px] font-bold tracking-wide text-emerald-950 uppercase flex items-center gap-1.5">
                                Sannidh WORM Secure Audit Seal
                                <Badge className="bg-emerald-600 text-white font-sans font-bold text-[7px] px-1 py-0 h-3 border-0">IMMUTABLE</Badge>
                              </p>
                              <p className="text-[8px] text-emerald-700 font-mono break-all leading-tight">
                                RECORD_HASH: SHA256/ea8294bd8294ab92bc92401bcda9001b92bc2948bd982141bc2<br />
                                BLOCKCHAIN_BLOCK: #4,928,103 | TIMESTAMP: {new Date().toISOString()}<br />
                                {filingARN && <span className="font-bold text-emerald-950">GOVERNMENT_FILING_ARN: {filingARN}</span>}
                              </p>
                            </div>
                          </div>
                        )}
...
const CADashboard = () => {
  ...
  const fetchClients = async () => {
    // Mock clients state is already set and preserved
  };

  // Fetch pending consent requests
  const fetchPendingRequests = async () => {
    // Bypassed for simulation
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-24 pb-16">
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <DemoSectorSelectorBar
            globalSector={globalSector}
            onSectorChange={(s) => {
              setGlobalSector(s);
              // When switching to a specific sector, also clear the per-client filter
              if (!s || s === 'general') {
            ...
            <Tabs value={activeZone} onValueChange={(val: any) => setActiveZone(val)} className="w-full">
              <div className="flex items-center justify-between mb-8 overflow-x-auto pb-2 scrollbar-none">
               <TabsList className="h-14 bg-card/40 border border-border/50 p-1 flex-shrink-0">
                    ...
                    {isZoneAllowed(globalSector, 'case-room') && (
