 ...
 * AGENT NETWORK PANEL
 * ===================
 * Advanced neural-network-style panel showing all 10 agents,
 * their connections, live status, cross-agent message feed,
 * and system health metrics.
 * Replaces the basic AgentStatusPanel for the Real Company Dashboard.
 */
...
import { 
  ...
  CheckCircle2, Folder, ChevronDown, ChevronUp, RefreshCw, Play,
  Pause, Activity, MessageSquare, Link2, ArrowRight, Radio,
  Network, Cpu, BarChart3, AlertTriangle
} from 'lucide-react';
import { useAgentOrchestrator, type AgentId, type AgentDefinition, type AgentMessage } from './CompanyAgentOrchestrator';
...
const MESSAGE_TYPE_CONFIG: Record<string, { color: string; icon: React.ComponentType<any>; label: string }> = {
...
const AgentCard = ({ agent, onTrigger, onPause, onResume }: { 
  ...
  return (
    ...
    >
      ...
      {/* Recent Cross-Wire Messages */}
      {recentMessages.length > 0 && (
        <div className="space-y-1.5 mb-3">
          {recentMessages.map((msg) => {
            const typeConfig = MESSAGE_TYPE_CONFIG[msg.type] || MESSAGE_TYPE_CONFIG.DATA_SYNC;
...
const CrossWireMessageFeed = ({ messages }: { messages: AgentMessage[] }) => {
  return (
    <div className="space-y-2.5 max-h-[420px] overflow-y-auto pr-2">
      {messages.slice(0, 15).map((msg, index) => {
        ...
        return (
          ...
          >
            <MsgIcon className={`w-4.5 h-4.5 ${typeConfig.color} shrink-0 mt-0.5`} />
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
...
export const AgentNetworkPanel = () => {
  ...
  return (
    ...
    >
      <Card className="border-purple-500/20 bg-gradient-to-br from-purple-500/5 via-background to-background overflow-hidden">
        <CardHeader className="pb-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <CardDescription className="text-sm mt-0.5">
                  Interconnected agents monitoring your compliance ecosystem in real-time
                </CardDescription>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className="flex rounded-lg border border-border/50 overflow-hidden">
                ...
                >
                  ...
                  {unacknowledgedMessages > 0 && (
                    <Badge className="ml-1.5 bg-primary/20 text-primary text-[10px] px-1.5 h-4">
          ...
          <div className="flex items-center gap-6 mt-4 pt-4 border-t border-border/30 text-xs text-muted-foreground">
            <div className="flex items-center gap-1.5">
              ...
              <span>System: <strong className="text-foreground">{state.systemUptime}%</strong> uptime</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Radio className="w-4 h-4 text-blue-400" />
              <span>Wire Messages: <strong className="text-foreground">{state.totalMessagesExchanged}</strong></span>
            </div>
            <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span>Agents: <strong className="text-green-400">{activeCount}</strong> active</span>
            ...
            >
              <CardContent className="pt-2">
                {activeView === 'grid' ? (
                  ...
                  <CrossWireMessageFeed messages={state.messages} />
                )}
              </CardContent>
            </motion.div>
          )}
