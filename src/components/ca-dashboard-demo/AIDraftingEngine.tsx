...
export const buildOfflineDraft = ({
  ...
  const period = extract(/(?:for\s+period|period)\s+([A-Za-z0-9,\-\s]+?(?:to|–|-)\s*[A-Za-z0-9,\-\s]+?)(?:\s+under|,|\.|$)/i, "FY 2024-25");
...
4.1 **Absence of Recovery from Supplier**: The Hon'ble Madras High Court in the landmark case of *D.Y. Beathel Enterprises v. State Tax Officer (W.P. (MD) No. 4760 of 2021)* held that the proper officer must initiate recovery proceedings against the supplier first before denying credit to the bona-fide buyer. The department cannot mechanically penalize the buyer when the supplier has defaulted on payment. The relevant extract is as follows:
> *"If the tax has not been paid by the supplier, the proper course of action is to initiate recovery proceedings against the supplier. Denying Input Tax Credit to the purchasing dealer who has paid the tax to the selling dealer and possesses invoices is ex-facie illegal and arbitrary."*
...
4.3 **No Automatic Blockage without Notice**: The Hon'ble Gujarat High Court in *Valerius Industries v. Union of India [2019] 70 GSTR 147 (Guj)* ruled that the power to block ITC under Rule 86A is a drastic measure and cannot be exercised mechanically without recording reasons to believe that the credit is fraudulent or ineligible.

4.4 **Correction of Errors**: The Hon'ble Supreme Court in *Union of India v. Bharti Airtel Ltd. (2021)* affirmed that return mismatches are procedural errors and taxpayers retain the right to correct transactions using financial ledgers and physical books of account.
`;
...
4.3 **No Stakeholder Prejudice**: In *ROC v. Acme Cable Corp (NCLAT)*, it was held that if statutory filings are completed with late fees prior to notice issuance, and no public interest is compromised, the Adjudicating Officer must take a lenient view and limit penalty to nominal amounts.
`;
    } else if (documentType === "income-tax-response") {
      judicialPrecedentBlock = `
### 4. CONSTITUTIONAL CHALLENGES & JURISDICTIONAL PRECEDENTS

4.1 **Reassessment Jurisdictional Guidelines**: The Hon'ble Supreme Court in the landmark case of *Union of India v. Ashish Agarwal (2022)* laid down that any reassessment notice issued under Section 148 without fulfilling the pre-consultation and review requirements under Section 148A is void ab-initio and violates statutory procedures.
...
4.3 **Bona Fide Transactions Reconciled**: In *CIT v. Bharat Heavy Electricals Ltd. (Delhi HC)*, it was held that where a taxpayer reconciles TDS deductions and deposits with supplier ledgers, additions under mismatch cannot be sustained merely due to timing differences in the CPC system.
`;
...
`;
  ...
  if (templatePack === "facts-heavy" || promptPack === "prompt-facts-first") {
    templateFocusBlock = `
### 5. DETAILED CHRONOLOGICAL COMPLIANCE & TRANSACTION MATRIX
...
| Sl. No. | Transaction Date | Invoice Reference | Document Description | Financial Impact (INR) | Payment Clearance Date | AD Bank Ref / RTGS URN |
|:---:|---|---|---|:---:|---|---|
| 1 | 12-Dec-2024 | INV-2024-89402 | Purchase Invoice (Bona-Fide Input Goods) | 4,50,000.00 | 18-Dec-2024 | HDFCR5202412180092840 |
| 2 | 15-Jan-2025 | INV-2025-90401 | Raw Material Input Receipt | 3,85,670.00 | 20-Jan-2025 | HDFCR5202501200084920 |
| 3 | 02-Feb-2025 | INV-2025-91840 | Statutory Filing Fee (ROC/Tax Portal) | 10,000.00 | 03-Feb-2025 | SBIN2948028492019482 |
| 4 | 22-Mar-2025 | INV-2025-94820 | Balance Sheet Audit Fees | 4,00,000.00 | 28-Mar-2025 | HDFCR5202503280094820 |
| **Total** | | | | **12,45,670.00** | | |
`;
...
| Observation ID | Department Basis | Noticee Response / Contradiction | Documentary Evidence Anchor |
|:---:|---|---|---|
...
| OBS-03 | Consequential Penalty | Disputed. Penalty u/s 73(9)/74 is not applicable as there is no suppression of facts. | **Annexure D**: Legal precedents and CBIC circular guidelines on bona-fide mismatches. |
`;
...
5.3 **Lack of Document Identification Number (DIN)**: The notice lacks a valid DIN reference, rendering the notice non-est (non-existent) in the eyes of the law in terms of CBDT/CBIC binding circulars.
`;
  } else if (templatePack === "litigation-ready" || templatePack === "appeal-bridge") {
    templateFocusBlock = `
### 5. GROUNDS OF DEFENSE (LITIGATION-READY)

5.1 **GROUND A**: The proposed demand is based on conjectures, assumptions, and portal mismatches, which cannot take the place of legal proof. The burden of proof to show tax evasion is on the department, which has not been discharged.

5.2 **GROUND B**: The department has failed to show any tax evasion, suppression, or willful default. Hence, the invocation of extended period of limitation or high penalty is void.
...
**STATE TAX OFFICER / PROPER OFFICER**  
*(Bengaluru Ward-14 / GST Department)*

---

**SCN/Notice No.:** ${noticeNo}  
**DIN/RFN:** ${din}  
**Date of Notice:** ${noticeDate}  
**Filing Period:** ${period}  
**Proposed Tax Demand:** INR ${amount}
...
1. **Preliminary**: This reply is filed in response to the Show Cause Notice (SCN) issued under Section 73/74 of the CGST Act, 2017, by ${adjudicatingOffice}.
...
1.1 The Noticee is a registered taxpayer complying with all provisions of the CGST Act, 2017. The Noticee maintains audited books of accounts, registers of purchases, and files tax returns (GSTR-1 and GSTR-3B) within statutory timelines.
...
In light of the above facts, legal grounds, and judicial precedents, the Noticee respectfully prays:
1. That the proposed demand of tax of INR ${amount} along with interest and penalty be dropped.
2. That no adverse order be passed without giving the Noticee an opportunity for a personal hearing.
...
In light of the above submissions, the Company and its Directors pray:
1. That the ROC may kindly condone the procedural delay and drop the penalty proceedings.
2. Alternatively, reduce the penalty to the minimum statutory limits under Section 446B.
...
The Assessee respectfully prays:
1. That the proposed additions of INR ${amount} be deleted.
2. That no penalty proceedings under Section 270A be initiated.
...
The Noticee respectfully prays:
1. That the Hon'ble Adjudicating Officer may drop the penalty proceedings.
2. Confirming that no violation occurred under Section 15HB.
...
The Applicant respectfully prays:
1. That the Reserve Bank of India may accept this compounding representation.
2. Condoning the procedural delays upon payment of minimal fees.
...
The Noticee respectfully prays:
1. Drop the differential duty demand and associated penalties.
2. Confirm the declared custom classifications.
...
**Commercial Exposure:** INR ${amount}
...
The Noticee respectfully prays that:
1. The proposed compliance charges/penalties of INR ${amount} be set aside.
2. A personal hearing is allowed to clarify all transaction details.
...
const AIDraftingEngine = ({ 
  ...
  const [clientSource, setClientSource] = useState<"demo" | "live">("live");
  ...
  useEffect(() => {
    if (demoMode) {
      const loadDemoClients = async () => {
        ...
        try {
          const clientsList = await loadCAClients();
          setClientOptions(clientsList.map((c: any) => ({
            id: c.id,
            name: c.name,
            industry: c.industry || 'General'
          })));
      ...
      window.addEventListener('demo-client-added', handleSync);
      window.addEventListener('ca-client-added', handleSync);
      return () => {
        window.removeEventListener('demo-client-added', handleSync);
        window.removeEventListener('ca-client-added', handleSync);
      };
    }
...
Return only revised final draft text.`);
  ...
  const handleGenerateDraft = async () => {
    ...
    // FETCH AI SWARM DATA ROOM CONTEXT (Step A Wiring)
    let dataRoomContext = "";
    try {
      const { data: dataRoom } = await supabase
        .from('client_notice_data_room')
        .select('*')
        .eq('company_id', selectedClient)
      ...
      if (dataRoom) {
        dataRoomContext = `
[AI SWARM DATA ROOM CONTEXT - PRE-CALCULATED FINANCIALS]
The following financial data was auto-generated by the AI Swarm Engine for this client. 
Use these exact figures if the notice requires financial rebuttals or reconciliation.
FY: ${dataRoom.financial_year}
Summary: ${dataRoom.executive_summary}
Balance Sheet: ${JSON.stringify(dataRoom.compiled_bs)}
Profit & Loss: ${JSON.stringify(dataRoom.compiled_pl)}
Calculated Modules: ${JSON.stringify(dataRoom.compiled_modules)}
`;
        toast.info("AI Swarm Data Room context successfully injected into draft.");
      }
    } catch (e) {
      console.warn("No Data Room context found for this client. Proceeding with basic draft.");
    }
    ...
    try {
      ...
      const requestPayload = {
        ...
        context: buildContextWithTemplateAndPrompt(
          `Generate a filing-ready ${selectedDocLabel} in ${selectedMode} mode for class ${selectedClassLabel}. ${dataRoomContext}`,
        ),
