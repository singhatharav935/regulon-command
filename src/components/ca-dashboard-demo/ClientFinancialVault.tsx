...
export default function ClientFinancialVault() {
  ...
  const handleTriggerSwarm = async () => {
    ...
    // Demo bypass: simulate swarm for demo-auto clients
    if (selectedClient.startsWith('demo-auto-')) {
      setSwarmJob({ status: 'running', progress: 0, current_step: 'Fetching bank feeds...' });
      const steps = [
        { progress: 20, step: 'Categorizing transactions...' },
        { progress: 40, step: 'Generating Balance Sheet...' },
        { progress: 60, step: 'Computing P&L Statement...' },
        { progress: 80, step: 'Calculating 26 statutory modules...' },
        { progress: 100, step: 'Finalizing Data Room' },
      ];
      let i = 0;
      const interval = setInterval(() => {
        if (i < steps.length) {
          setSwarmJob({ status: 'running', progress: steps[i].progress, current_step: steps[i].step });
          i++;
        } else {
          clearInterval(interval);
          setSwarmJob({ status: 'completed', progress: 100, current_step: 'All 26 modules compiled' });
          localStorage.setItem(`swarm_completed_${selectedClient}`, 'true');
          setTriggering(false);
          toast.success("AI Swarm Completed!", { description: "Data Room populated with all 26 modules + BS + P&L." });
          fetchDataRoom();
