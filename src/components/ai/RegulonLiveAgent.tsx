 ...
 * 
 * Features:
 * - Wake word detection ("Hey Regulon")
...
interface RegulonLiveAgentProps {
  dashboardId: string;
...
const RESPONSES = {
  greeting: [
    ...
    "Good morning! I'm Regulon, your AI compliance partner. How can I help you today?",
    "Hello! Regulon here. Ready to work on your compliance tasks."
  ],
  balance_sheet: [
...
const RegulonLiveAgent = ({
  dashboardId,
  dashboardType,
  userName = 'User',
  companyName = 'Your Company'
}: RegulonLiveAgentProps) => {
  // State
  ...
  const startListening = useCallback(() => {
    ...
    recognition.onresult = (event: any) => {
      ...
      // Check for wake word "Hey Regulon"
      const fullText = (final + interim).toLowerCase();
      if (fullText.includes('hey regulon') || fullText.includes('regulon')) {
        setIsWakeWordActive(true);
      }
      
      // Process final result
      if (final && isWakeWordActive) {
        const command = final.replace(/hey regulon/gi, '').trim();
        if (command.length > 2) {
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xl font-bold text-white">Regulon AI</span>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {isListening ? '🎤 Listening...' : isWakeWordActive ? '✨ Ready' : '💤 Say "Hey Regulon"'}
            </Badge>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              >
                ...
                {isListening ? 'Listening...' : 'Talk to Regulon'}
              </Button>
...
export default RegulonLiveAgent;
