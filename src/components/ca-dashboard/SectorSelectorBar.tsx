/**
 * SectorSelectorBar — REAL version
 * src/components/ca-dashboard/SectorSelectorBar.tsx
 *
 * Horizontal bar of professional text-only sector pills for ExternalCADashboardReal.
 * EXCLUSIVELY for the real dashboard — demo dashboard has its own separate copy.
 */

import { type ClientSector, SECTOR_CONFIGS } from "@/lib/client-sector";

interface RealSectorSelectorBarProps {
  globalSector: ClientSector | null;
  onSectorChange: (sector: ClientSector | null) => void;
}

const REAL_SECTOR_PILLS: { id: ClientSector | null; label: string }[] = [
...
export function RealSectorSelectorBar({
  globalSector,
  onSectorChange,
}: RealSectorSelectorBarProps) {
  return (
    <div className="flex items-center gap-2 mt-4 mb-2 overflow-x-auto pb-1 scrollbar-none">
      ...
      {REAL_SECTOR_PILLS.map(({ id, label }) => {
