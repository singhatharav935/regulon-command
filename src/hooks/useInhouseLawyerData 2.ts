    ...
    mutationFn: async ({
      ...
      const { error: reqError } = await supabase
        ...
        .update({
          ...
          lawyer_comments: lawyerComments,
          reviewed_at: now,
        })
        .eq("id", requestId);
      if (reqError) throw new Error(reqError.message);

      // Sync draft_runs.status
      const draftStatus =
      ...
      const { error: draftError } = await supabase
        ...
        .eq("id", draftRunId);
      if (draftError) console.warn("[LawyerReview] Draft status sync error:", draftError.message);
...
export const useSendDraftForReview = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      companyId: string;
      sentBy: string;
      priority?: LawyerReviewRequest["priority"];
      caNotes?: string;
    }) => {
...
export const useLawyerContracts = (companyId: string | null) => {
  return useQuery({
    queryKey: ["lawyer-contracts", companyId],
...
export const useAddContract = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["lawyer-contracts", variables.company_id] });
    },
  });
};
...
export const useLawyerCases = (companyId: string | null) => {
  return useQuery({
    queryKey: ["lawyer-cases", companyId],
...
export const useLawyerNotices = (companyId: string | null) => {
  return useQuery({
    queryKey: ["lawyer-notices", companyId],
...
export const useLawyerRisks = (companyId: string | null) => {
  return useQuery({
    queryKey: ["lawyer-risks", companyId],
