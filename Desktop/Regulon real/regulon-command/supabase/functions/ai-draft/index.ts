...
const getCorsHeaders = (req: Request) => {
  ...
  const allowOrigin = allowlist.length === 0
    ? "*"
    : ((localOriginAllowed && isLocalOrigin) || hasWildcard || isAllowlisted)
      ? origin
      : "null";
...
type LogicLevel = "sannidh_core" | "sannidh_nexus_9" | "sannidh_sovereign";
...
const normalizeLogicLevel = (value: unknown): LogicLevel | null => {
  ...
  if (normalized === "sannidh_core" || normalized === "core" || normalized === "quick_draft") return "sannidh_core";
  if (normalized === "sannidh_nexus_9" || normalized === "nexus-9" || normalized === "expert_analysis") return "sannidh_nexus_9";
  if (normalized === "sannidh_sovereign" || normalized === "sovereign" || normalized === "supreme_research") return "sannidh_sovereign";
  return null;
};
...
const inferLogicLevelFromDraftMode = (draftMode: string | null | undefined): LogicLevel => {
  ...
  if (normalized === "conservative") return "sannidh_core";
  if (normalized === "aggressive") return "sannidh_sovereign";
  return "sannidh_nexus_9";
};

const getLogicLevelDescription = (logicLevel: LogicLevel): string => {
  const profiles: Record<LogicLevel, string> = {
    sannidh_core: "SANNIDH_CORE™: Foundational reliability and administrative speed. Keep response concise, procedural, and filing-safe. Avoid deep judicial expansion unless strictly needed.",
    sannidh_nexus_9: "SANNIDH_NEXUS-9™: Professional-grade expert analysis. Execute para-wise statutory mapping and robust legal rebuttal with precise section-rule anchoring.",
    sannidh_sovereign: "SANNIDH_SOVEREIGN™: Supreme research depth. Use maximum judicial reasoning depth, include litigation-grade positioning, and strengthen formal legal-submission architecture.",
  };
        ...
        if (!shouldRetry) {
          return new Response(response.body, {
            ...
            headers: {
              ...
              "x-sannidh-model-used": activeModel,
              "x-sannidh-fallback-used": String(modelIndex > 0),
              "x-sannidh-attempt-count": String(attempts),
              "x-sannidh-model-router-version": MODEL_ROUTER_VERSION,
            },
          });
        }
  ...
  if (lastResponse) {
    return new Response(lastResponse.body, {
      ...
      headers: {
        ...
        "x-sannidh-model-used": chain[chain.length - 1] ?? "gpt-4.1-mini",
        "x-sannidh-fallback-used": String(chain.length > 1),
        "x-sannidh-attempt-count": String(attempts),
        "x-sannidh-model-router-version": MODEL_ROUTER_VERSION,
      },
    });
...
Schema:
{
  ...
  "missing_for_final_filing": string[]
    ...
    return new Response(JSON.stringify({
      ...
      metadata: {
        aiProvider: aiConfig.provider,
        aiModelUsed: response.headers.get("x-sannidh-model-used") ?? aiConfig.model,
        aiFallbackUsed: response.headers.get("x-sannidh-fallback-used") === "true",
        aiAttemptCount: Number(response.headers.get("x-sannidh-attempt-count") ?? "1"),
