...
router.post('/approve-task', async (req, res) => {
  try {
    ...
    // 2. If approved, generate final PDF with REGULON seal
    ...
    res.json({
      ...
      message: action === 'approve' 
        ? 'Task approved. PDF with REGULON AI seal generated.'
...
router.post('/generate-document', async (req, res) => {
  try {
    ...
    const processingTime = logicLevel === 'regulon_sovereign' ? 8000 :
                          logicLevel === 'regulon_nexus_9' ? 5000 : 3000;
    ...
    const draftContent = generateLegalDraft({
      ...
      noticeDetails,
      logicLevel: logicLevel || 'regulon_core',
      noticeClass
    });
    ...
    res.json({
      ...
      metadata: {
        processingEngine: logicLevel === 'regulon_sovereign' ? 'REGULON_SOVEREIGN™' : 
                         logicLevel === 'regulon_nexus_9' ? 'REGULON_NEXUS-9™' : 'REGULON_CORE™',
        confidenceScore: qaReport.filing_score,
        generatedBy: 'REGULON AI Drafting Engine v3.0',
        timestamp
      }
    });
  } catch (error) {
...
═══════════════════════════════════════════════════════════════════
              ...
              Generated by REGULON AI Drafting Engine v3.0
                     ${logicLevel?.toUpperCase() || 'REGULON_CORE'}
═══════════════════════════════════════════════════════════════════
...
Logic Level: ${logicLevel || 'REGULON_CORE'}
