...
```bash
sudo cp deploy/systemd/sannidh-agent.service /etc/systemd/system/sannidh-agent.service
```

2. Create env file:

```bash
sudo mkdir -p /etc/sannidh
sudo cp .env.agent /etc/sannidh/sannidh-agent.env
```
...
sudo systemctl status sannidh-agent
```
