...
SANNIDH uses two monitoring tools:
...
// Test error
try {
  throw new Error("Test error from SANNIDH");
} catch (error) {
  captureException(error as Error, { context: "Testing" });
}

// Test message
captureMessage("Test message from SANNIDH", "info");
```
...
function TestSentry() {
  const testError = () => {
    try {
      ...
      throw new Error("Test error from SANNIDH frontend");
    } catch (error) {
