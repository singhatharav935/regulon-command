...
- [ ] **Frontend Deployment (Vercel / AWS Amplify)**
  ...
  - Inject the production base URL (e.g., `VITE_CA_API_BASE_URL=https://api.sannidh.com/v1`) into the Vercel Environment Variables.
...
- [ ] **Domain Binding (DNS Configuration)**
  - Purchase domain (e.g., `sannidh.in` or `sannidh.com`).
  - Configure `A` records and `CNAME` targeting your Frontend Vercel IP.
  - Configure a subdomain (`api.sannidh.com`) targeting the Backend Render/AWS IP.
...
- [ ] **CORS Security LockDown**
  ...
  - Explicitly lock CORS to ONLY accept requests from `https://app.sannidh.com`.
