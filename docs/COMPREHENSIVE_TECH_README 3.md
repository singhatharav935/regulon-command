# SANNIDH: Comprehensive Technical Documentation
...
11. [Why CAs & Companies Choose SANNIDH](#why-choose-sannidh)
...
### What is SANNIDH?
**One-Liner**: SANNIDH is an AI-powered regulatory intelligence and compliance automation platform that helps Chartered Accountants, companies, and law firms stay compliant with India's 7 regulatory portals (MCA, GST, IT, RBI, SEBI, Stock Exchange, Pensions) through real-time monitoring, automated drafting, and intelligent task prioritization.
...
**What it's used for**: Display the 7 regulatory portals SANNIDH monitors with coverage details
    ...
    .on("INSERT", (payload) => {
      if (payload.new.type === "regulatory_news") {
        ...
        if (Notification.permission === "granted") {
          new Notification("SANNIDH Alert", {
...
SANNIDH's AI Swarm uses a **coordinator-agent model** with 4 specialized agents working in sequence:
...
app.use(helmet());
app.use(cors({ origin: ["https://sannidh.in", "https://app.sannidh.in"] }));
```
...
const generateJWT = (userId, role) => {
  return jwt.sign(
    ...
    { expiresIn: "1h", issuer: "sannidh", audience: "api" }
  );
};
```
...
// Restart on crash
pm2 start server.js --name "sannidh-api" --restart-delay 5000 --max-restarts 10;
```
...
ab -n 10000 -c 1000 https://api.sannidh.in/api/tasks
...
- **SANNIDH**: Built specifically for Indian regulations (MCA, GST, RBI, SEBI, Stock Exchange, IT, Pensions)
...
- CA recommendations embedded (if CA uses SANNIDH for your work, company naturally adopts it)
- Integration with bank/GST portals (automatic data sync, no manual entry)

**2. Network Effects**
- CA uses SANNIDH → recommends to company clients
- Companies see other companies in audit → FOMO, adopt SANNIDH
...
- API integrations (if company connected SANNIDH to accounting software)
...
| Feature | SANNIDH | Traditional CA Firm | Big 4 Audit | Generic Compliance Tool |
|---------|---------|-------------------|------------|-------------------------|
...
**SANNIDH** is a best-in-class compliance SaaS platform leveraging:
...
**File Location**: `/Users/atharavsingh/Desktop/SANNIDH_MASTER/frontend/COMPREHENSIVE_TECH_README.md`
